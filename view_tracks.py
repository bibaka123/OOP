import customtkinter as ctk  # Importing the customtkinter library for modern GUI components
...
def set_text(text_area, content):
    text_area.delete("1.0", ctk.END)  # Clear the content of the text area
...
class TrackViewer:
    def __init__(self, window):  # Constructor method for initializing the GUI
        ...
        ctk.set_appearance_mode("light")  # Set the appearance mode (light/dark)
        ctk.set_default_color_theme("blue")  # Set the default color theme

        # Create and position a button to list all tracks
        list_tracks_btn = ctk.CTkButton(window,height=40, text="List All Tracks", command=self.list_tracks_clicked)
        ...
        check_track_btn = ctk.CTkButton(window, height=40, text="View Track", command=self.view_tracks_clicked)
        check_track_btn.grid(row=0, column=3, sticky="WE", padx=10, pady=10)

        # Create and position a scrolled text widget for listing all tracks
        self.list_txt = ctk.CTkTextbox(window, width=400, height=200)
        self.list_txt.grid(row=1, column=0, columnspan=3, sticky="W", padx=10, pady=10)

        # Create and position a text widget for displaying individual track details
        self.track_txt = ctk.CTkTextbox(window, width=300, height=100)
...
if __name__ == "__main__":  # Check if the script is being run as the main module
    window = ctk.CTk()  # Create the main customtkinter window object
    ...
    window.mainloop()  # Start the customtkinter event loop to display the GUI and handle events
